...
int main(int argc, char* argv[])
{
    ...
    FLAGS_log_dir = "/home/chris/myspace/thunder/log/";
    LOG(INFO) << "INFO";  
    LOG(INFO) << "INFO1";  
    LOG(WARNING) << "WARNING";  
    ...
    LOG_INFO("thunder server listen on %s:%d", cfg._ip.c_str(), cfg._port);
    LOG_INFO("server thread pool size:%d", cfg._net_nthreads);

    // 加载so
    if (0 == load_so(cfg._app_so.c_str()))
    {
        int ret = thunder_dll.thunder_handle_init((void *)cfg._app_conf.c_str());
        if (ret != 0)
        {
