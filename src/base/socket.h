#ifndef ROCKET_BASE_SOCKET_H_
#define ROCKET_BASE_SOCKET_H_
...
public:
    ...
    // Set the _fdCLOEXEC flag if value is nonzero,
    ...
    bool Ioctl(int cmd, int* value) {
        return ::ioctl(_fd, cmd, reinterpret_cast<void*>(value)) == 0;
    }
 ...
 public:
    static int GetLastError() {
        return rocket::concurrency::ThisThread::GetLastError();
    }
    static std::string GetLastErrorString() {
        return rocket::concurrency::ThisThread::GetLastErrorString();
    }

    protected:
    static void SetLastError(int error) {
        rocket::concurrency::ThisThread::SetLastError(error);
    }
