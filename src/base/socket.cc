...
bool Socket::Create(int af, int type, int protocol/* = 0*/) {
    Close();
...
bool Socket::SetCloexec(bool value) {
    ...
    return fcntl(_fd, F_SETFD, flags) >= 0;
}
...
bool Socket::Bind(const EndPoint& end_point) {
    ...
    return ::bind(_fd, &address, address_len) == 0;
}
...
bool Socket::GetLocalEndPoint(EndPoint* end_point) const {
    ...
    if (getsockname(_fd, &addr, &addr_length) == 0) {
...
bool Socket::GetPeerEndPoint(EndPoint* end_point) const {
    struct sockaddr addr;
    socklen_t addr_length;
    if (getpeername(_fd, &addr, &addr_length) == 0) {
        end_point->FromSockAddr(addr, addr_length);
        return true;
    }
    return false;
...
bool Socket::WaitReadable(int64_t timeout_in_millisecond, bool restart) {
    while (true) {
        int n = PollReadable(_fd, timeout_in_millisecond);
        if (n != -1) {
            return n > 0;
...
bool Socket::WaitWriteable(int64_t timeout_in_milliseconds, bool restart) {
    while (true) {
        int n = PollWriteable(_fd, timeout_in_milliseconds);
        if (n != -1) {
            return n > 0;
        } else if (!IsInterruptedAndRestart(restart)) {
            break;
...
bool ListenSocket::Accept(Socket* socket,
    ...
    while (true) {
        ...
        Debug("Accept ret:%d, fd:%d", ret, fd());
        if (ret != -1) {
            socket->Attach(ret);
            if (end_point) {
